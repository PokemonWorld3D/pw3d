...
public static class TCP2_Utils
{
	...
	static public string FindReadmePath(bool relativeToAssets = false)
	{
		...
		if(paths.Length > 0)
		{
			string firstPath = System.IO.Path.GetDirectoryName( paths[0] ).Replace(@"/", @"\");
			if(relativeToAssets)
			{
				firstPath = firstPath.Replace(@"\", @"/").Replace(Application.dataPath, "");
			}
...
//			EditorUtility.CopySerialized(originalMesh, newMesh);
	...
	static public string UnityToSystemPath(string path) { return path.Replace("/", @"\"); }
	static public string SystemToUnityPath(string path) { return path.Replace(@"\", "/"); }
}
